import { MENU_FOOTER, MENU_EN } from '@/utils/const'
...
const Index = ( props ,ref) => {
  ...
  const beforeInjection = (svg) => {
    svg.setAttribute('style', 'color: rgba(255,255,255,0.5)')
  }

  const MENU = useMemo(()=>{
    return i18n.language==='zh'?MENU_FOOTER:MENU_EN
  },[i18n])
    ...
    return MENU.map((item, idx)=>{
      ...
      <ul key={item.menu} className='text-white text-left sm:w-[208px] sm:text-sm'>
        <li className='py-[22px] border-solid border-white/[.15] border-b flex justify-between sm:border-none sm:py-4'>
          ...
          <NoSSR>
            <ReactSVG className='w-5 h-5 svg sm:hidden' beforeInjection={beforeInjection} src={visible?'/svg/minus.svg':'/svg/add.svg'} onClick={()=>setSubmenuIdx(visible?-1:idx)}/>
          </NoSSR>
        </li>
      ...
      <div className='content-auto'>
        ...
        <div className='flex flex-col items-center'>
          <div className='flex flex-col items-center sm:flex-row sm:w-full'>
            ...
            <p className='text-xs sm:order-2 sm:flex-1 sm:text-right'>{t('label.address')}: {t('address')}</p>
            <div className='order-4' ref={popoverRef}  onMouseLeave={()=>setOpenPopover(false)}>
              <Popover
                isOpen={openPopover}
                positions={['top']}
              ...
              >
                <svg onMouseEnter={()=>setOpenPopover(true)} className='hidden fill-white/[.65] w-4 h-4 sm:block' width="16" height="16" viewBox="0 0 16 16" xmlns="http://www.w3.org/2000/svg">
