...
import { MENU_HEADER, MENU_EN } from '@/utils/const'
import RatioBox from '../ratioBox/index'
import { Link, Responsive, LanguageSwitch } from '@/components/index'
import { Popover } from 'react-tiny-popover';
import { useTranslation } from 'next-i18next'

const hoverStyle = `sm:hover:text-[#FC0407] transition-all ease-in-out duration-[280ms] cursor-pointer`
...
const Index = ({transparent:transparentProp = false}, ref) => {
  ...
  const MENU = useMemo(()=>{
    return i18n.language==='zh'?MENU_HEADER:MENU_EN
  },[i18n])
  ...
  const submenus = useMemo(()=>MENU?.[submenuIdx]?.submenu,[MENU, submenuIdx])
  ...
  const renderMenu = () => MENU.map((menu, idx)=> {
    return (
      menu.external?(
        <a className={hoverStyle} href={menu.path??''} target={menu.target??''}>{menu.menu}</a>
      ):menu.path?(
        <Link className={hoverStyle} href={menu.path??''} target={menu.target??''}>{menu.menu}</Link>
      ): 
      <div key={menu.menu} className={hoverStyle} >
        {menu.menu}
      </div>
    )
  })
  ...
  const renderMenuDetail = () => MENU.map((item, idx)=> {
    ...
    return (<ul key={item.name} className='flex flex-col text-black/70 text-xs basis-16'>
      {submenu?.map(menu=>(
        <li key={menu.name} className='mt-4 -mr-96 '>
          {
            menu.external?(
              <a className={hoverStyle} href={menu.path??''} target={menu.target??''}>{menu.name}</a>
            ):(
              <Link className={hoverStyle} href={menu.path??''} target={menu.target??''}>{menu.name}</Link>
            )
          }
          {menu.list?.length?
            <ul className='leading-[22px] list-disc pl-5 mt-3'>
              {
                menu.list.map(ele=>(<Link key={ele.name} href={ele.path}><li className={hoverStyle}>{ele.name}</li></Link>))
              }
            </ul>:null
          }
        </li>
  ...
  const renderMenuMobile = () => MENU.map((item, idx)=>{
    const submenu = item.submenu
    return   (<>
      <div key={item.menu} className='text-[22px]/[22px] font-semibold text-black/90' onClick={()=>(!isPC)&&submenu&&setSubmenuIdx(idx)}>
        {
          submenu?(
            item.menu
  ...
  const renderSubmenuMobile = () => submenus?.map(item=>
    (<div key={item.name} className='text-[22px]/[22px] font-semibold text-black/90'>
     <Link href={item.path??''} target={item.target??''}>{item.name}</Link>
    </div>))
    ...
    <div className={`${transparent?'sm:bg-transparent sm:text-white':'visible text-black/90'} group font-medium fixed w-screen bg-white/90 duration-300 ease-in-out z-10 sm:hover:text-black/90 sm:hover:bg-white/90`} >
      <div ref={ref}>
        <div className="content-auto px-5 py-3.5 sm:px-0 sm:py-5 flex justify-between items-center border-black/[.15] border-b sm:border-0">
          ...
          <div className='hidden sm:block flex items-center' ref={popoverRef} onMouseLeave={()=>setOpenPopover(false)}>
            <span className=''>4008-299-666</span>
            <Popover
              isOpen={openPopover}
              positions={['bottom']}
              parentElement={popoverRef.current}
              content={
                <img className='w-28 h-28 z-[9999]' src='/images/contact-us/wechat.png' alt="" />
              }
            >
              <div className='w-6 h-6 bg-white/30 group-hover:bg-white group-[.visible]:bg-white  rounded-full inline-flex justify-center items-center ml-3' onMouseEnter={()=>setOpenPopover(true)}>
